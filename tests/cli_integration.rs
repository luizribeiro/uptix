...
#[test]
fn test_discover_no_existing_lock() {
    let temp_dir = TempDir::new().unwrap();
    ...
    let output = Command::new(uptix_binary())
        ...
        .arg("discover")
        .output()
        .expect("Failed to execute uptix discover");

    assert!(output.status.success());
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("1 new dependencies discovered:"));
    assert!(stdout.contains("postgres:15"));
    assert!(stdout.contains("Fetching metadata for new dependencies..."));
    assert!(stdout.contains("Wrote uptix.lock successfully"));
...
#[test]
fn test_discover_with_existing_dependencies() {
    let temp_dir = TempDir::new().unwrap();

    // Create a nix file with two dependencies
    let nix_content = r#"{
        postgres = uptix.dockerImage "postgres:15";
        redis = uptix.dockerImage "redis:latest";
    }"#;
    ...
    // Create initial lock file with only postgres
    let lock_content = r#"{
  "postgres:15": {
    "metadata": {
      "name": "postgres",
    ...
    "lock": {
      "imageDigest": "sha256:somehash",
      "sha256": "sha256-nixhash"
    }
  }
}"#;
    fs::write(temp_dir.path().join("uptix.lock"), lock_content).unwrap();
    ...
    assert!(!stdout.contains("postgres:15")); // Already tracked, shouldn't be listed as new
    assert!(stdout.contains("Fetching metadata for new dependencies..."));
    assert!(stdout.contains("Wrote uptix.lock successfully"));

    // Verify lock file now contains both dependencies
    let lock_path = temp_dir.path().join("uptix.lock");
    let updated_lock = fs::read_to_string(&lock_path).unwrap();
    assert!(updated_lock.contains("postgres:15"));
    assert!(updated_lock.contains("redis:latest"));
}

#[test]
fn test_discover_all_dependencies_tracked() {
    let temp_dir = TempDir::new().unwrap();
    ...
    // Create lock file with the dependency already tracked
    let lock_content = r#"{
  "postgres:15": {
    "metadata": {
      "name": "postgres",
    ...
    assert!(output.status.success());
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("All dependencies are already tracked in uptix.lock."));
}

#[test]
fn test_discover_no_dependencies_in_codebase() {
    let temp_dir = TempDir::new().unwrap();

    // Create a nix file with NO uptix dependencies
    ...
    assert!(output.status.success());
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("No uptix dependencies found in the codebase."));
}
