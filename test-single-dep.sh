...
if uptix update --dependency "luizribeiro/hello-world-rs" 2>&1 | grep "Found 1 dependencies matching" > /dev/null; then
...
if uptix update --dependency "luizribeiro/hello-world-rs:main" 2>&1 | grep "Found 1 dependencies matching" > /dev/null; then
...
if uptix list | grep "Dependencies in uptix.lock:" > /dev/null; then
...
if uptix show "postgres:15" | grep "Dependency Key: postgres:15" > /dev/null; then
